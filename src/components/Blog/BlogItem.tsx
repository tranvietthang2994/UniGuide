...
import { simpleTextFormatter } from "@/lib/textFormatter";
...
const BlogItem = ({ blog }: { blog: BlogPost }) => {
  ...
  const getImageSrc = () => {
    if (imageError) {
      return "/images/blog/blog-1.png"; // Fallback to default
    }

    if (blog.thumbnailUrl) {
      // Check if it's a real URL (not example.com)
      if (blog.thumbnailUrl.includes("example.com")) {
        return "/images/blog/blog-1.png"; // Use default for example URLs
      }
      return blog.thumbnailUrl;
    }

    return "/images/blog/blog-1.png"; // Default fallback
  };
    ...
    <article className="group overflow-hidden rounded-[15px] bg-white shadow-dropdown dark:bg-gray-dark">
      ...
      <div className="px-6 py-7">
        <div className="mb-3 flex flex-wrap items-center gap-6">
          <div className="flex items-center gap-2 font-satoshi text-sm font-medium -tracking-[0.1px] dark:text-gray-5">
            <svg
        ...
        <div
          className="w-full dark:text-gray-5"
          dangerouslySetInnerHTML={{
            __html: simpleTextFormatter(getExcerpt(blog.content, 100)),
          }}
        />
      </div>
    </article>
